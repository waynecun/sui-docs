...
slug: /move-by-example/custom-transfer
---

In Sui Move, objects defined with only the `key` ability can not be transferred by default. To enable transfers, the publisher has to create a custom transfer function. This function can include any arguments, for example a fee, that users have to pay to transfer.
