...
Welcome to the guide for making remote procedure calls (RPC) to the Sui network. This document walks you through how to connect and interact with the Sui network using the Sui JSON-RPC API. Use the RPC layer to send your dApp transactions to [Sui validators](../../contribute/nodes/validator.mdx) for verification.

This guide is useful for developers interested in Sui network interactions via API and should be used in conjunction with the [SuiJSON format](sui-json-format.mdx) for aligning JSON inputs with Move Call arguments.

For a similar guide on Sui network interactions via CLI, refer to the [Sui Client CLI](../../build/setup/cli/client-cli.mdx) documentation.

Follow the instructions to [install Sui binaries](../../build/setup/cli/install-sui.mdx#install-sui-binaries).

## Connect to a Sui network {#connect-sui}

You can connect to a Sui Full node on a Sui network. Follow the guidance in the [Connect to a Sui Network](../../build/setup/connect-to-a-network.mdx) topic to start making RPC calls to the Sui network.

To configure your own Sui Full node, see [Configure a Sui Full node](../../contribute/nodes/full-node.mdx).

## Sui SDKs

You can sign transactions and interact with the Sui network using any of the following:

- [Sui Rust SDK](../sdk/rust-sdk.mdx), a collection of Rust language JSON-RPC wrapper and crypto utilities.
...
## Sui JSON-RPC examples {#examples-json-rpc}
...
#### Create an unsigned transaction to transfer a Sui coin from one address to another {#unsigned-tx}
...
#### Sign a transaction using the Sui keytool {#sign-tx-keytool}
...
#### Execute a transaction with a serialized signature {#serialized-sig}
...
Native transfer by `sui_transferObject` supports any object that allows for public transfers. Some objects cannot be transferred natively and require a [Move call](#sui_movecall). See [Transactions](../../learn/core-concepts/transactions.mdx#native-transaction) for more information about native transfers.

### Invoke Move functions

The example command in this section demonstrate how to call Move functions.

#### Execute a Move call transaction {#call-transaction}
...
the [Move](../../build/create-smart-contracts/smart-contracts.mdx) language):
...
function is described in more detail in the [Sui CLI client](../../build/setup/cli/client-cli.mdx#calling-move-code) documentation.
...
To learn more about which `args` a Move call accepts, see [SuiJSON](../json-rpc/sui-json-rpc.mdx).
...
sure the package is valid. If some modules have [initializers](../../build/create-smart-contracts/debug-and-publish.mdx#module-initializers), these initializers execute in Move (which means new Move objects can be created in the process of publishing a Move package). Gas budget is required because of the need to execute module initializers.
