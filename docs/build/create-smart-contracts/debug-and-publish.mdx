...
For functions in a Sui Move package to actually be callable from Sui (rather than an emulated Sui execution scenario), you have to publish the package to the Sui [distributed ledger](../../learn/core-concepts/how-sui-works.mdx), where it is represented as an immutable Sui object.
...
[publish](../setup/cli/client-cli.mdx#publish-packages) Move code and to [call](../setup/cli/client-cli.mdx#calling-move-code) that code. See the [Sui CLI client documentation](../setup/cli/client-cli.mdx) for information on how to publish the package created in this tutorial.
