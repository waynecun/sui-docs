...
Any transaction that requires access to a `Clock` must go through [consensus](../../learn/core-concepts/consensus-engine.mdx) because the only available instance is a [shared object](../../learn/core-concepts/objects.mdx#shared). As a result, this technique is not suitable for transactions that must use the single-owner fast-path (see [Epoch timestamps](#epoch-timestamps) for a single-owner-compatible source of timestamps).
