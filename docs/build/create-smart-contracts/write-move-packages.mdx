...
Before you can build a Sui Move package and run its code, you must first [install Sui binaries](../setup/cli/install-sui.mdx#install-or-update-sui-binaries) for your operating system.
...
Running the previous command creates a directory with the name you provide (`my_first_package`). The command populates the new directory with a skeleton Sui Move project that consists of a `sources` directory and a [`Move.toml` manifest](move-toml.mdx). Open the manifest with a text editor to review its contents:
