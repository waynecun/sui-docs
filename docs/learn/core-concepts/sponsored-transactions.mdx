...
## Potential Risks Using Sponsored Transactions {#risks}

The most significant potential risk when using sponsored transactions is [equivocation](../../reference/sui-framework/sui-glossary.mdx#equivocation). In some cases under certain conditions, a sponsored transaction can result in all associated owned objects, including gas in a locked state when examined by Sui validators. To avoid double spending, validators lock objects as they validate transactions. An equivocation occurs when an owned object’s pair (`ObjectID`, `SequenceNumber`) is concurrently used in multiple non-finalized transactions.
...
## Create a user-initiated sponsored transaction {#user-initiated}
...
## Create a sponsor-initiated sponsored transaction {#sponsor-initiated}
...
## Create sponsored transactions using a GasData object {#gasdata-object}
...
## Create a Sui Gas Station {#gas-station}
...
### Sui Gas Station Risk Mitigation {#gas-station-risk-mitigation}
...
## Code examples to create a Sui Gas Station {#example-gas-station}
...
### User-initiated sponsored transactions {#user-initiated-gas-station}
...
### Sponsored transactions with GasData objects {#gasdata-gas-station}
...
### User and Sponsor-initiated transaction. {#user-and-sponsor-gas-station}
...
## Sponsored Transaction Data Structure {#data-structure}
...
To learn more about Transaction in Sui, see [Transactions](../../learn/core-concepts/transactions.mdx).
