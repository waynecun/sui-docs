...
To learn how to set up and run a Sui Validator node, including how staking and rewards work, see [Sui Validator Node](../../contribute/nodes/validator.mdx).
...
## The role of Narwhal and Bullshark {#narwhal-bullshark}

Sui takes advantage of [Narwhal and Tusk: A DAG-based Mempool and Efficient BFT Consensus](../core-concepts/consensus-engine.mdx) and the Tusk successor [Bullshark](https://arxiv.org/abs/2201.05677). Narwhal/Bullshark (N/B) are also being implemented in Sui so that when Byzantine agreement is required it uses a high-throughput DAG-based consensus to manage shared locks while execution on different shared objects is parallelized.
...
- Transactions take objects as input and produce objects as output&mdash;check out the [objects](../../learn/core-concepts/objects.mdx) section to learn more about the structure and attributes of objects.
- Sui supports several different transaction types&mdash;see the [transactions](../../learn/core-concepts/transactions.mdx) section for full details.
