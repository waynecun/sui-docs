...
## Step 1: Add keys to Sui keystore {#step-1}
...
## Step 2: Create a MultiSig address {#step-2}
...
## Step 3: Send objects to a MultiSig address {#step-3}

This example requests gas from a local network using the default URL following the guidance in [Sui Local Network](../../build/setup/local-network.mdx).
...
## Step 4: Serialize ANY transaction {#step-4}
...
## Step 5: Sign the transaction with two keys {#step-5}
...
## Step 6: Combine individual signatures into a MultiSig {#step-6}
...
## Step 7: Execute a transaction with MultiSig {#step-7}
