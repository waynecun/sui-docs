...
The Sui Client CLI installs when you install Sui. See the [Install Sui](./install-sui.mdx) topic for prerequisites and installation instructions.
...
You can start the client in two modes: interactive shell or command line interface [Configure Sui client](../connect-to-a-network.mdx#configure-sui-client).
...
- `--args` - a list of function arguments formatted as
  [SuiJSON](../../../reference/json-rpc/sui-json-format.mdx) values (hence the preceding `0x` in address
  and object ID):
...
You must publish packages to the Sui [distributed ledger](../../../learn/core-concepts/how-sui-works.mdx#architecture) for the code you developed to be available in Sui. To publish packages with the Sui client, use the `publish` command.

Refer to the [Move developer documentation](../../create-smart-contracts/smart-contracts.mdx) for a
description on how to [write a simple Move code package](../../create-smart-contracts/write-move-packages.mdx#create-the-package),
...
the part of Move developer documentation concerning [module initializers](../../create-smart-contracts/debug-and-publish.mdx#module-initializers) for more details.

You might notice that the gas object that was used to pay for
publishing was updated as well.

**Important:** If the publishing attempt results in an error regarding verification failure, [build your package locally](../../create-smart-contracts/build-and-test.mdx#building-a-package) (using the `sui move build` command) to get a more verbose error message.
