...
The previous chapter, [Dynamic Fields](dynamic-fields.mdx), introduced a way to extend existing objects with _dynamic fields_. Note that it's possible to delete an object that still has (potentially non-`drop`) dynamic fields. This might not be a concern when adding a small number of statically known additional fields to an object, but is particularly undesirable for _on-chain collection types_ which could be holding unboundedly many key-value pairs as dynamic fields.
...
Collections are built on top of [dynamic fields](dynamic-fields.mdx), and so are subject to its [limitations](dynamic-fields.mdx#current-limitations). Additionally, the following functionality is planned, but not currently supported:
