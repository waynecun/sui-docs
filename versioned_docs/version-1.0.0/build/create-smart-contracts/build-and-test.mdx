...
[Move testing framework](https://github.com/move-language/move/blob/main/language/documentation/book/src/unit-testing.mdx) that enables you to write unit tests that analyzes Move code much like test frameworks for other languages (e.g., the built-in [Rust testing framework](https://doc.rust-lang.org/rust-by-example/testing/unit_testing.html) or the [JUnit framework](https://junit.org/) for Java).
...
If you execute this command for the package created in [write a package](write-move-packages.mdx), you see the following output. Unsurprisingly,
...
[transaction](../../learn/core-concepts/transactions.mdx), and you might want to test interactions between different transactions within a single test (for example, one transaction creating an
...
that provides Sui-related testing functionality otherwise unavailable in pure Move and its [testing framework](https://github.com/move-language/move/blob/main/language/documentation/book/src/unit-testing.mdx).
...
Update your `my_module.move` file to include [entry functions](./smart-contracts.mdx#entry-functions) callable from Sui that implement sword creation and transfer. With these in place, you can then add a multi-transaction test that uses the `test_scenario` module to test these new capabilities. Put these functions after the accessors (Part 5 in comments).
...
The code of the new functions uses struct creation and Sui-internal modules (`TxContext` and `Transfer`) in a way similar to what you have seen in the previous sections. The important part is for the entry functions to have correct signatures as described in [Write Smart Contracts with Sui Move](./smart-contracts.mdx#entry-functions).
